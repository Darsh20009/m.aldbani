...
async function callGoogleAuth(access_token: string): Promise<{ token: string; user: User }> {
  ...
  if (!res.ok) {
    ...
    throw new Error((err as { error?: string }).error || "فشل تسجيل الدخول بـGoogle");
  }
...
export default function Login() {
  ...
  const handleGoogleSuccess = async (access_token: string) => {
    ...
    try {
      const result = await callGoogleAuth(access_token);
