...
export default function Login() {
  ...
  // Google's SDK throws synchronously if VITE_GOOGLE_CLIENT_ID isn't set at build
  ...
  const handleGoogleSuccess = async (accessToken: string) => {
    ...
    try {
      ...
      toast({ title: "فشل تسجيل الدخول بـGoogle", description: err instanceof Error ? err.message : "حاول مجدداً", variant: "destructive" });
    } finally {
  ...
  useEffect(() => {
    ...
    if (params.get("error") === "google") {
      toast({
        title: "فشل تسجيل الدخول بـGoogle",
  ...
  return (
    <div className="min-h-screen flex" dir={isRTL ? "rtl" : "ltr"} style={{ background: "#FAF8F4" }}>
      ...
      <div className="hidden lg:flex flex-col w-[46%] relative overflow-hidden"
        ...
        <motion.div animate={{ scale: [1, 1.3, 1], x: [0, 40, 0], y: [0, -20, 0] }}
          transition={{ duration: 20, repeat: Infinity, ease: "easeInOut" }}
          className="absolute orb-blue"
          style={{ width: 700, height: 700, top: "-25%", left: "-25%", opacity: 0.6 }} />
        <motion.div animate={{ scale: [1, 1.2, 1], y: [0, 50, 0] }}
      ...
      <div className="flex-1 flex flex-col justify-center items-center px-6 py-16 overflow-y-auto">
        <motion.div initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="space-y-3 mb-6">
            {/* Google — client-side popup flow via @react-oauth/google.
                Only mounted when a client ID is configured for this deploy;
                otherwise show a disabled-looking button that explains why. */}
            {hasGoogleClientId ? (
              <GoogleLoginButton
                onSuccess={handleGoogleSuccess}
                onError={handleGoogleError}
                loading={googleLoading}
                label={t("Continue with Google", "المتابعة بـ Google")}
                loadingLabel={t("Signing in…", "جاري التحقق…")}
              />
            ) : (
              <motion.button
                type="button"
                onClick={() => toast({ title: t("Google sign-in unavailable", "تسجيل الدخول بـGoogle غير متاح حالياً") })}
                whileHover={{ scale: 1.01, y: -1 }}
                whileTap={{ scale: 0.98 }}
                className="w-full flex items-center justify-center gap-3 h-12 rounded-xl font-semibold text-sm bg-white transition-all hover:shadow-md opacity-60"
          ...
          <div className="flex rounded-xl overflow-hidden mb-6 p-1"
            ...
            {([
              ...
              <motion.button key={id} type="button" onClick={() => switchMode(id)}
                className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-bold transition-colors relative"
                style={{ color: mode === id ? "#0F172A" : "#94A3B8" }}>
                {mode === id && (
                  <motion.div layoutId="tab-bg" className="absolute inset-0 rounded-lg bg-white"
