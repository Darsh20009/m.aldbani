    ...
    <AdminLayout>
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-white/60">Manage consulting offerings and pricing.</p>
        </div>
          ...
          <TableBody>
            {isLoading ? (
              <TableRow className="border-white/10">
                <TableCell colSpan={4} className="text-center py-8 text-white/50 animate-pulse">Loading services...</TableCell>
              </TableRow>
            ) : services.length === 0 ? (
              <TableRow className="border-white/10">
                <TableCell colSpan={4} className="text-center py-8 text-white/50">No services found.</TableCell>
              </TableRow>
            ) : (
              services.map((service) => (
                <TableRow key={service.id} className="border-white/10 hover:bg-white/5">
                  ...
                  <TableCell className="text-white/70">{service.price || "Custom"}</TableCell>
