    ...
    <AdminLayout>
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-white/60">Manage prospective clients and inquiries.</p>
        </div>
          ...
          <TableBody>
            {isLoading ? (
              <TableRow className="border-white/10">
                <TableCell colSpan={6} className="text-center py-8 text-white/50 animate-pulse">Loading leads...</TableCell>
              </TableRow>
            ) : leads.length === 0 ? (
              <TableRow className="border-white/10">
                <TableCell colSpan={6} className="text-center py-8 text-white/50">No leads found.</TableCell>
              </TableRow>
            ) : (
              leads.map((lead) => (
                <TableRow key={lead.id} className="border-white/10 hover:bg-white/5">
                  ...
                  <TableCell className="text-white/70 capitalize">{lead.source}</TableCell>
                  <TableCell>
